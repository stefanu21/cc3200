...
#define APPLICATION_NAME        "Krippenlicht"
...
#define LED_POS_DIM_STRING		"POS"


#define SERVICE_NAME            "Krippe._http._tcp.local"
...
#define ROLE_INVALID                    (-5)
#define AUTO_CONNECTION_TIMEOUT_COUNT   (1000)   /* 5 Sec */
...
unsigned char POST_DIM_token[] = "__SL_P_DIM";
                               ...
                               SlHttpServerResponse_t *pSlHttpServerResponse)
{
    ...
    switch (pSlHttpServerEvent->Event) {
        ...
        case SL_NETAPP_HTTPPOSTTOKENVALUE_EVENT:
        {
          ...
          if(memcmp(ptr, POST_DIM_token, strlen((const char *)POST_DIM_token)) == 0)
          {
            ...
            ptr += strLenVal;
            led = *ptr;
            strLenVal = strlen(LED_POS_DIM_STRING);
			ptr += 2;  // nr + _
...
static long ConfigureSimpleLinkToDefaultState()
{
    ...
    if (ROLE_STA != lMode)
    {
        ...
        if (ROLE_STA != lRetVal)
        {
        	Report("device not in station mode");
...
static void HTTPServerTask(void *pvParameters)
{
    ...
    lRetVal = sl_NetAppMDNSUnRegisterService(SERVICE_NAME,\
                    (unsigned char)strlen(SERVICE_NAME));
    lRetVal = sl_NetAppMDNSRegisterService(SERVICE_NAME,\
                    (unsigned char)strlen(SERVICE_NAME),\
                    "path=/led_demo.html;srcvers=1D90645",\
                    (unsigned char)strlen("path=/led_demo.html;srcvers=1D90645"),
                    80,2000,1);
